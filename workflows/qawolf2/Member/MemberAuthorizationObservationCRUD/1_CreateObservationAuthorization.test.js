...
test('CreateObservationAuthorization', async ({ page: _page }) => {
    //--------------------------------
    // Arrange:
    //--------------------------------

    const loginID = `ObservationCRUD`;
    const password = `8VSzI$^QsXuq57HI554555`;
    const member = {
    ...
    // Clean-up previously-created authorization
    ...
    try {
        await page.locator(`[name="auth_provider_2_site_id"] ~ button[title="Lookup"]`).click({ timeout: 3000 });
    } catch {
    ...
    await expect(page.getByText(`Observation Auth #`)).toBeVisible();
    ...
    await expect(page.getByText(`* Observation Start Date: ${todaysDate} 12:00:00 AM`)).toBeVisible();
...
async function selectDropdownOption(page, inputSelector, optionText) {
    ...
    await page.waitForTimeout(300); // let autocomplete process
    await input.press('Enter');     // select the value
...
test('CreateObservationAuthorization', async ({ page: _page }) => {
    ...
    // Inpatient Status
    await selectDropdownOption(page, `input[name="aush_inpatient_status_id__1_input"]`, patientStatus);

    // Click X button
    await page.getByRole(`button`, { name: `` }).nth(2).click();

    // Auth Status
    await selectDropdownOption(page, `input[name="aush_status_id__1_input"]`, authStatus);
    ...
    // Team
    await selectDropdownOption(page, `input[name="auth_team_reference_id_input"]`, team);

    // Reviewer
    await selectDropdownOption(page, `input[name="auth_reviewer_user_id_input"]`, loginID);
