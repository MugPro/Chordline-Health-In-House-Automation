...
export async function cleanupTabOnMembersPage(page, options = {}) {
    ...
    // ✅ Wait for grid container (NOT tbody)
    await page.locator(gridId).waitFor({ state: 'visible', timeout: 10000 });

    // Locate rows containing loginID
    let rowLocator = page
        .locator(`${gridId} table tbody tr`)
    ...
    console.log(`Found ${initialCount} record(s) for ${loginID}. Deleting...`);

    while (await rowLocator.count() > 0) {
        const firstRow = rowLocator.first();

        const deleteButton = firstRow.locator('button.k-grid-delete');

        // ✅ Skip safely if delete button not present
        if (await deleteButton.count() === 0) {
            console.log('Row found but no delete button — stopping cleanup.');
            break;
        }
        ...
        // Recalculate rows (Kendo re-renders DOM)
        rowLocator = page
            .locator(`${gridId} table tbody tr`)
            .filter({ hasText: loginID });
    }
...
test('Delete all Observation Authorizations', async () => {
    ...
    // Optional: Just log remaining (no failure if 0)
    const remainingRows = await page
        .locator(`${gridId} table tbody tr`)
        .filter({ hasText: loginID })
        .count();
...
export async function cleanupTabOnMembersPage(page, options = {}) {
    ...
    // Use your WORKING selector
    let rowLocator = page.locator(
        `${gridId} table tbody tr:visible:has-text("${loginID}")`
    );

    // ✅ If no rows exist, exit safely
    if (await rowLocator.count() === 0) {
        console.log(`No records found for ${loginID}. Nothing to delete.`);
        return;
    }

    console.log(`Deleting records for ${loginID}...`);
    ...
    while (await rowLocator.count() > 0) {
        ...
        // Handle workflow lock notification
        const workflowNotification = page.locator('#notif-message', {
            hasText: 'This record is locked by Workflow Rule Account.',
        });
        ...
        // Re-query rows after Kendo re-render
        rowLocator = page.locator(
            `${gridId} table tbody tr:visible:has-text("${loginID}")`
        );
    }
