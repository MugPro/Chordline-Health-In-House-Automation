...
test.describe('Authorization Config - Default Diagnosis = First Diagnosis', () => {
    ...
    test('Creating a new Bed Day pre-populates the FIRST Diagnosis when default = First Diagnosis', async () => {
        //--------------------------------
        // Arrange:
        //--------------------------------
        ...
        await page
            ...
            .click();
        await page.getByRole('option', { name: 'First Diagnosis' }).click();
        ...
        await expect(
            page.locator(
                `[data-collection-code="COLLECTION_AUTH_LINE_ITEM_DIAGNOSIS"]:has-text("${diag1}")`
            )
        ).toBeVisible();
