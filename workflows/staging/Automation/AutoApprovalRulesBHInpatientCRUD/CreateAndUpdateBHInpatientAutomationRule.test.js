...
test.describe('BH Inpatient Automation Rules — Create & Update', () => {
    test('Create a BH Inpatient rule, verify all fields, update rule, and re-verify', async () => {
        //--------------------------------
        // Arrange:
        //--------------------------------
        const ruleName = faker.lorem.words(3); // FIX: faker.random.words -> faker.lorem.words
        const ruleNameEdited = `${ruleName} - edited`;
        const loginID = `BHInpatientCRUDAutoApprovalRules`;
        ...
        try {
            ...
            const allBirthGenderOptions = await page.locator(`[role="region"] [role="option"]:visible`).allInnerTexts();
            const selectedBirthGender = faker.helpers.arrayElement(allBirthGenderOptions); // FIX: faker.random.arrayElement -> faker.helpers.arrayElement
            await page
            ...
            await page.locator(`[role="region"] [role="option"]:visible`).first().waitFor();
            const allBenefitPlanOptions = await page.locator(`[role="region"] [role="option"]:visible`).allInnerTexts();
            ...
            await page
                ...
                .click({ delay: 500 });
            await page.locator(`[role="region"] [role="option"]:visible`).first().waitFor();
            const allAgeOperatorOptions = await page.locator(`[role="region"] [role="option"]:visible`).allInnerTexts();
            ...
            // Fill Age input
            const randomAge = faker.number.int({ min: 18, max: 80 }).toString(); // FIX: faker.datatype.number -> faker.number.int
            ...
            const allInsuranceCompanyOptions = await page
                ...
                .allInnerTexts();
            const selectedInsuranceCompany = faker.helpers.arrayElement(allInsuranceCompanyOptions); // FIX
            await page
                .locator(
            ...
            await page
                .locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has(:text-is("Cards:")) input`
                )
                .click({ delay: 500, force: true });
            await page.locator(`[role="option"]:visible`).first().waitFor();
            const allCardOptions = await page.locator(`[role="region"] [role="option"]:visible`).allInnerTexts();
            ...
            if (memberPCP) {
                await page
                    .locator(
                        `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Member's PCP:") input`
                    )
                    .check();
            }
            ...
            if (inMemberNetwork) {
                await page
                    .locator(
                        `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("In Member's Network:") input`
                    )
                    .check({ delay: 500, force: true });
            }
            ...
            await page
                .locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Specialties")`
                )
                .click({ delay: 500, force: true });
            await page.locator(`[role="region"] [role="option"]:visible`).first().waitFor();
            ...
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedSpecialties}"):visible`)
                .click();
            ...
            await expect(async () => {
                await page
                    ...
                    .click({ delay: 500, force: true });
                await page.locator(`[role="region"] [role="option"]:visible`).first().waitFor({ timeout: 3500 });
            ...
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedAPCard}"):visible`)
                .click();

            // Select "Member's PCP"
            const memberAPPCP = randomBool();
            if (memberAPPCP) {
                await page
                    .locator(
            ...
            // Select "In Member's Network:"
            const inMembersNetworkAP = randomBool();
            ...
            const allAPSpecialtiesOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedAPSpecialties = faker.helpers.arrayElement(allAPSpecialtiesOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedAPSpecialties}"):visible`)
                .click();
            await page
                .locator(
            ...
            const allOPProviderRoleOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedOPProviderRole = faker.helpers.arrayElement(allOPProviderRoleOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedOPProviderRole}"):visible`)
                .click();
            ...
            // Click `Preferred` from dropdown menu
            await page.locator(`[role="region"] [role="option"] :text-is("Preferred"):visible`).click();
            ...
            const allSpecialtiesOPOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedSpecialtiesOP = faker.helpers.arrayElement(allSpecialtiesOPOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedSpecialtiesOP}"):visible`)
                .click();
            ...
            const allServiceReqTypeOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedServiceReqType = faker.helpers.arrayElement(allServiceReqTypeOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedServiceReqType}"):visible`)
                .click();
            ...
            const allRequestTypeOptions = await page
                ...
                .allInnerTexts();
            const selectedRequestType = faker.helpers.arrayElement(allRequestTypeOptions); // FIX
            await page
                .locator(
            ...
            if (selectedRequestType === 'Bed Day') {
                ...
                const allRequestedBedLevelOptions = await page
                    .locator(`[role="region"] [role="option"]:visible`)
                    .allInnerTexts();
                let selectedRequestedBedLevel1 = faker.helpers.arrayElement(allRequestedBedLevelOptions); // FIX
                await page
                    .locator(
            ...
            if (selectedRequestType === 'Code') {
                ...
                const allRequestedCodeOptions = await page
                    .locator(`[role="region"] [role="option"]:visible`)
                    .allInnerTexts();
                let selectedRequestedCode1 = faker.helpers.arrayElement(allRequestedCodeOptions); // FIX
                await page
                    .locator(`[role="region"] [role="option"] :text-is("${selectedRequestedCode1}"):visible`)
                    .click();
            ...
            const allDiagnosisCodeOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedDiagnosisCode = faker.helpers.arrayElement(allDiagnosisCodeOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedDiagnosisCode}"):visible`)
                .click();
            ...
            const allMaxReqUnitTypeOptions = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedMaxReqUnitType = faker.helpers.arrayElement(allMaxReqUnitTypeOptions); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedMaxReqUnitType}"):visible`)
                .click();
            ...
            await page.getByRole(`dialog`, { name: `Manage Automation` }).getByPlaceholder(`Search...`).fill(ruleName);
            ...
            await expect(page.locator(`[name="aaru_provider_1_network_status_id_input"]`)).toHaveValue(`Preferred`);

            // Assert that Birth Gender is correct
            await expect(page.locator(`#aaru_birth_gender_id`)).toHaveText(selectedBirthGender);
            ...
            await expect(
                page.locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Cards") span:text-is("${selectedCard}")`
                )
            ).toBeVisible();
            ...
            if (inMemberNetwork) {
                await expect(
                    page.locator(
                        `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("In Member's Network") [type="checkbox"]`
                    )
                ).toBeChecked();
            }

            // Assert Network Status
            await expect(page.locator(`#aaru_provider_1_network_status_id`)).toHaveText(`Preferred`);

            // Assert Specialties
            await expect(
                page.locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Specialties") span:text-is("${selectedSpecialties}")`
                )
            ).toBeVisible();
            ...
            await expect(page.locator(`#aaru_provider_other_role_id`)).toHaveText(selectedOPProviderRole);
            ...
            // Assert Network Status
            await expect(page.locator(`#aaru_provider_other_network_status_id`)).toHaveText(`Preferred`);
            ...
            //--------------------------------
            // Arrange: Remove all options with an "X"
            //--------------------------------
            ...
            const allBirthGenderOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedBirthGender2 = faker.helpers.arrayElement(allBirthGenderOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedBirthGender2}"):visible`)
                .click();
            ...
            const allBenefitPlanOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedBenefitPlan2 = faker.helpers.arrayElement(allBenefitPlanOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedBenefitPlan2}"):visible`)
                .first()
                .click();
            ...
            await expect(async () => {
                await page
                    ...
                    .click({ delay: 500 });
                await page.locator(`[role="region"] [role="option"]:visible`).first().waitFor({ timeout: 2500 });
            }).toPass({ timeout: 30 * 1000 });
            const allAgeOperatorOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedAgeOperator2 = faker.helpers.arrayElement(allAgeOperatorOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedAgeOperator2}"):visible`)
                .click();
            ...
            const allInsuranceCompanyOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedInsuranceCompany2 = faker.helpers.arrayElement(allInsuranceCompanyOptions2); // FIX
            await page
                .locator(
            ...
            const allDeceasedOptions2 = await page
                ...
                .allInnerTexts();
            const selectedDeceased2 = faker.helpers.arrayElement(allDeceasedOptions2); // FIX
            await page
                .locator(
            ...
            // Click `Preferred` from dropdown menu
            await page
                .locator(`[data-bind="attr: { class: fields.aaru_provider_1_network_status_id.inputClass }"] [role="button"]`)
                .first()
                .click();
            ...
            const allSpecialtiesOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedSpecialties2 = faker.helpers.arrayElement(allSpecialtiesOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedSpecialties2}"):visible`)
                .click();
            ...
            // Select "In Member's Network:"
            const inMembersNetworkAP2 = randomBool();
            ...
            const allAPSpecialtiesOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedAPSpecialties2 = faker.helpers.arrayElement(allAPSpecialtiesOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedAPSpecialties2}"):visible`)
                .click();
            await page
                .locator(
            ...
            const allOPProviderRoleOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedOPProviderRole2 = faker.helpers.arrayElement(allOPProviderRoleOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedOPProviderRole2}"):visible`)
                .click();
            ...
            const allSpecialtiesOPOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedSpecialties2OP = faker.helpers.arrayElement(allSpecialtiesOPOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedSpecialties2OP}"):visible`)
                .click();
            ...
            const allServiceReqTypeOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedServiceReqType2 = faker.helpers.arrayElement(allServiceReqTypeOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedServiceReqType2}"):visible`)
                .click();
            ...
            const allRequestTypeOptions2 = await page
                ...
                .allInnerTexts();
            const selectedRequestType2 = faker.helpers.arrayElement(allRequestTypeOptions2); // FIX
            await page
                .locator(
            ...
            // Select Requested Bed Level
            let selectedRequestedBedLevel2; // ensure not shadowed inside block
            if (selectedRequestType2 === 'Bed Day') {
                ...
                const allRequestedBedLevelOptions2 = await page
                    .locator(`[role="region"] [role="option"]:visible`)
                    .allInnerTexts();
                selectedRequestedBedLevel2 = faker.helpers.arrayElement(allRequestedBedLevelOptions2); // FIX
                await page
                    .locator(
            ...
            if (selectedRequestType2 === 'Code') {
                ...
                const allRequestedCodeOptions2 = await page
                    .locator(`[role="region"] [role="option"]:visible`)
                    .allInnerTexts();
                selectedRequestedCode2 = faker.helpers.arrayElement(allRequestedCodeOptions2); // FIX
                await page
                    .locator(
            ...
            const allDiagnosisCodeOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedDiagnosisCode2 = faker.helpers.arrayElement(allDiagnosisCodeOptions2); // FIX
            await page
                .locator(`[role="region"] [role="option"] :text-is("${selectedDiagnosisCode2}"):visible`)
                .click();
            ...
            const allMaxReqUnitTypeOptions2 = await page
                .locator(`[role="region"] [role="option"]:visible`)
                .allInnerTexts();
            const selectedMaxReqUnitType2 = faker.helpers.arrayElement(allMaxReqUnitTypeOptions2); // FIX
            await page
                .locator(
            ...
            expect(authType2).toBe('IP-BH');

            // Click updated rule name (FIX: click edited name, not the original)
            ...
            // Assert that Birth Gender is correct
            await expect(page.locator(`#aaru_birth_gender_id`)).toHaveText(selectedBirthGender2);
            ...
            await expect(
                page.locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Cards") span:text-is("${selectedCard2}")`
                )
            ).toBeVisible();
            ...
            await expect(
                page.locator(
                    `[class*="formSection columns"]:has-text("Admitting Hospital") [class*="outerfield"]:has-text("Specialties") span:text-is("${selectedSpecialties2}")`
                )
            ).toBeVisible();
            ...
            // Assert In Member's Network (Other Provider checkbox verification used in your original snippet)
            if (inMemberNetwork2OP) {
                await expect(page.locator(`[id="aaru_provider_other_is_members_network-23"]`)).toBeChecked({ timeout: 3000 });
            }

            // Assert Network Status
            await expect(page.locator(`#aaru_provider_2_network_status_id`)).toHaveText(`Preferred`);
            ...
            // OTHER PROVIDER:
            // Assert Provider role
            await expect(page.locator(`#aaru_provider_other_role_id`)).toHaveText(selectedOPProviderRole2);
            ...
            if (selectedRequestType2 === 'Bed Day') {
                await expect(async () => {
                    await page.getByText(`Bed Day`, { exact: true }).scrollIntoViewIfNeeded();
                    await expect(page.locator(`span:text-is("${selectedRequestedBedLevel2}"):visible`).first()).toBeVisible({
                        timeout: 2500
                    });
