...
test.describe(
    'Notes Detail Summary Report – Generate, Download, Print, Email',
    () => {
        test('Full workflow succeeds with correct data', async () => {
            ...
            //--------------------------------
            // Constants
            const loginID = `NotesDetailSummary`;
            ...
            const report = {
                ...
                noteReason: 'Member Activity',
                expectedText:
                    'Provides a description of Notes within the Date Range as specified including the Status, Reason,',
            };

            const generatedReportType = 'Notes Detail Summary Report';
            ...
            const bodyText = `Notes Detail Summary email test ${Date.now()}`;
            ...
            //--------------------------------
            // Generate Notes Detail Summary Report
            //--------------------------------
            ...
            await clickAndWait(
                ...
                //await page.getByRole('gridcell', { name: 'Notes Detail Summary Report' }).click();
            ...
            const formattedStart = dateFns.format(
            ...
            await clickAndWait(
                page,
                page.locator(`[role="grid"] [title="${formattedStart}"]`),
            );
            ...
            await clickAndWait(
                page,
                page.locator(
                    '[aria-autocomplete="list"]:near(:text("Note Status")) >> nth=0',
                ),
            );
            ...
            await clickAndWait(
                page,
                page
                    .getByRole('option', { name: report.noteStatus })
                    .locator('span'),
            );
            ...
            await clickAndWait(
                page,
                page.locator(
                    '[aria-autocomplete="list"]:near(:text("Note Reason")) >> nth=0',
                ),
            );

            await clickAndWait(
                page,
                page.getByRole('option', { name: report.noteReason }),
            );
            ...
            await expect(
                page.getByText(
                    `Member Notes ${member.firstName} ${member.lastName}`,
                ),
            ).toBeVisible();
            ...
            console.log(
                '✅ Notes Detail Summary Report: Generate, Download, Print, and Email (UI-confirmed) succeeded',
            );
        });
    },
);
